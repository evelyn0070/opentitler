...
div
  ...
  .widescreen(v-if="display.config" :class="{ showclassname:display.control.debug}" :style="allstyles")
    ...
    transition(mode="out-in" enter-active-class="animate__animated animate__slideInRight" leave-active-class="animate__animated animate__slideOutRight" )
      People(v-cname :key="display.people? display.people.name :''" :data="display.people" v-if="display.control.people && display.people")

    Title(v-cname :data="display.title" :control="display.control")

    Ticker(:data="display.ticker" :control="display.control")
    ...
    emoji-rain(:text="emojiText" :display="display")
    global-chats(:token="display.control && display.control.zoomsense_token" :display="display")
    barrage(:display="display")
    vote-charts()
</div>
...
import EmojiRain from "@/components/display/EmojiRain";
import GlobalChats from "./display/GlobalChats";
import Barrage from "@/components/display/Barrage";
import VoteCharts from "./charts/VoteCharts";
...
export default {
  ...
  computed:{
    ...
    emojiText(){
      const {content,control} = this.display;
      if (!content || !control){
        return ''
      }
      let index = control.currentcontent;
  ...
  watch: {
    ...
    display(newData){
      console.log('display change',newData)
    }
  }
}
</script>
